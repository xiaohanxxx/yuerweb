...
class Follow(models.Model):
    follower = models.ForeignKey(User, related_name='follower', on_delete=models.CASCADE)  # 关注者
    followed = models.ForeignKey(User, related_name='followed', on_delete=models.CASCADE)  # 被关注者
        ...
        for followeder in followeders:
            ...
            if t:
                user_followed.append(
                    {'username': followeder.follower.username, 'userid': followeder.follower.id, 'mutualfollower': 0}) # 0表示互相关注
            else:
                user_followed.append(
                    {'username': followeder.follower.username, 'userid': followeder.follower.id, 'mutualfollower': 1}) # 1表示未关注粉丝
